...
this_script_root = join(data_root,'Rangeland')
...
class Expand_points_to_rectangle:
    ...
    def run(self):
        ...
        site_info_dict = T.dict_zip(name_list,point_list)
        new_point_list = []
        new_name_list = []
        selected_site = ['US-Vcm','US-Vcp']
        for site in site_info_dict:
            if not site in selected_site:
                continue
            new_point_list.append(site_info_dict[site])
            new_name_list.append(site)
        rectangle_list = self.expand_points_to_rectangle(new_point_list)
        self.write_rectangle_shp(rectangle_list,new_name_list)
        pass
...
class Download_from_GEE:
    ...
    def __init__(self):
        ...
        self.collection = 'projects/rap-data-365417/assets/vegetation-cover-v3' # derived from Sentinel
        # --------------------------------------------------------------------------------
        self.this_class_arr, self.this_class_tif, self.this_class_png = T.mk_class_dir(
            f'Download_from_GEE',
            this_script_root, mode=2)
    ...
    def bands_list(self):
        band_name_list = ['AFG', 'BGR', 'LTR', 'PFG', 'SHR', 'TRE']
